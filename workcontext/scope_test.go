...
func TestHasScopeAnswersFromTheLastActorsGrant(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := workcontext.HasScope(wc, tc.kind, tc.action, tc.ids...); got != tc.want {
...
func TestEffectiveScopesFollowTheActorChain(t *testing.T) {
	// With an actor: the actor's attenuated grant, not the owner's authority.
	wc := verifiedClaims(t)
	effective := workcontext.EffectiveScopes(wc)
	...
	owner, err := newVerifier(t, workcontext.Config{Keys: staticKeys()}).Verify(context.Background(), token.Encoded())
	if err != nil {
	...
	if got := workcontext.EffectiveScopes(owner); len(got) != len(owner.GetAuthorityScopes()) {
...
func TestRequireScopeSaysWhyAndCannotBeWidenedAfterVerify(t *testing.T) {
	...
	err := workcontext.RequireScope(wc, "lastlogin:reports", "export", "r-1")
	...
	if err := workcontext.RequireScope(wc, "lastlogin:secrets", "read"); !errors.Is(err, workcontext.ErrInvalid) {
		t.Errorf("widened claims authorized: %v", err)
	}
	if workcontext.HasScope(wc, "lastlogin:logins", "read") {
		t.Error("a structurally invalid context should grant nothing, even what it used to")
	}

	if err := workcontext.RequireScope(nil, "lastlogin:logins", "read"); !errors.Is(err, workcontext.ErrInvalid) {
		t.Errorf("nil claims: %v", err)
	}
}

func TestRequireScopeFromContext(t *testing.T) {
	if err := workcontext.RequireScopeFromContext(context.Background(), "lastlogin:logins", "read"); !errors.Is(err, workcontext.ErrMissing) {
		t.Errorf("no context: %v, want ErrMissing", err)
	}

	ctx := workcontext.NewContext(context.Background(), verifiedClaims(t))
	if err := workcontext.RequireScopeFromContext(ctx, "lastlogin:logins", "read"); err != nil {
		t.Errorf("granted scope: %v", err)
	}
	if err := workcontext.RequireScopeFromContext(ctx, "lastlogin:reports", "read"); !errors.Is(err, workcontext.ErrDenied) {
		t.Errorf("restricted grant, kind-wide ask: %v, want ErrDenied", err)
	}
	if err := workcontext.RequireScopeFromContext(ctx, "lastlogin:reports", "read", "r-1"); err != nil {
...
func TestFromContextIsFalseForNilAndAbsent(t *testing.T) {
	...
	if wc, ok := workcontext.FromContext(workcontext.NewContext(context.Background(), verifiedClaims(t))); !ok || wc.GetTenantId() != "org-1" {
