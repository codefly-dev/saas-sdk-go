...
// solution service that receives a `codefly.work-context/v1` capability —
// minted by accounts, forwarded by whoever acts on the user's behalf — verifies
// it here before trusting a single claim.
//
// sdk-go owns the wire format and the hard parts: signature, time, and issuer
// checks (codefly.WorkContextVerifier) and the rotation-aware, fail-closed
// JWKS cache (codefly.WorkContextJWKSVerifier). This package is the thin layer
// a callee actually wires up: it points those primitives at the accounts JWKS
// through the same Gateway seam the client facades use, pins the callee's own
// service name as the audience, stores the verified claims on the request
// context, and maps failures onto HTTP, Connect, and gRPC status codes.
//
...
//
//	// in a handler
//	wc, _ := workcontext.FromContext(r.Context())
//	if err := workcontext.RequireScope(wc, "lastlogin:logins", "read"); err != nil {
...
var (
	...
	// and JWKS failure wraps it. A caller checks it with errors.Is.
...
// — the same shape the accounts and datasource facades ask for, so
// *github.com/codefly-dev/solution-runtime-go.Gateway satisfies it as-is and
// the runtime takes no dependency on this package.
...
// kid, time window, issuer, and finally that the audience is this service.
// The returned claims are canonical and safe to authorize from.
//
