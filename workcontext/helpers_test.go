...
// verifiedContext is the ctx a handler holds after the middleware ran.
func verifiedContext(t *testing.T) context.Context {
	t.Helper()
	wc, err := newVerifier(t, workcontext.Config{Keys: staticKeys()}).Verify(context.Background(), mintValid(t))
	if err != nil {
		t.Fatalf("Verify: %v", err)
	}
	return workcontext.NewContext(context.Background(), wc)
}

// wrap hides a sentinel behind an unrelated error, so a test proves the
// mapping helpers unwrap with errors.Is rather than matching on identity.
